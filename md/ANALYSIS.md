...
The same shared analyses (sections 01, 04–13, 16–22) run for both datasets; sections 02, 03, 14, 15 are Nevo-specific.
...
Use it to identify the spec that fits the moments best, and to sanity-check that the implied `price_coef` is negative and reasonable. Numbering jumps from 01 to 04 because 02 and 03 are Nevo-only.

## 02 — Demographic expansion effect (Nevo only)
...
## 03 — X2 characteristic comparison (Nevo only)
...
## 10 — Own-price elasticity summary by spec
...
## 12 — Top substitutes per product
...
## 13 — Cross-price elasticity asymmetry
...
## 14 — Cross-spec Spearman correlation (Nevo only)
...
## 15 — Within-firm vs between-firm substitution (Nevo only)
...
## 16 — Own-price elasticity cross-spec stability
...
## 17 — Cross-price elasticity cross-spec stability
...
The cross-spec analogue of analysis 16, but for cross-elasticities on the pairs that matter most in the preferred model.

## 18 — Pairwise spec agreement (MAD matrix)
...
Use it as a complement to analysis 14 (Spearman) — MAD picks up level differences that rank correlation misses.

## 19 — Objective aggregation across specifications
...
## 20 — Elasticity pair across all simulations (single spec)
...
## 21 — Elasticity pair across specifications (best simulation per spec)

`elasticity_pair_best_sim_across_specs(elas_rows, all_rows, label)` (analyze_results.py:1046). Mirror of analysis 20, but the cross-section is over specifications instead of seeds:
...
## 22 — Price coefficient across simulations
...
A diagnostic that complements analysis 22's mean: if α has a large range or is bimodal across seeds (a common pattern when there are two basins), the "best" α is much less informative.
...
Most of these analyses have a graphical counterpart in `plot_results.py`, written to `results/<dataset>/graphs/NN_<name>.png`. Sections 09 and 14/15 are text-only; section 03 is plotted only when matching specs exist.
