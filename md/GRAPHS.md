...
## 02 — `02_demographic_expansion.png` (Nevo only)
...
## 03 — `03_x2_comparison.png` (Nevo only)
...
## 05 — `05_multistart_stability.png`
...
This is the most diagnostic plot for basin structure. Vertical bands of points at the same α with very different objectives = repeated convergence to the same parameters from different random starts. Horizontal bands = different α reaching the same objective (parameter-identification problem). Combined with `06_convergence_audit.txt` you can read off which seeds populate each basin.

## 07 — `07_global_minimum.png`
...
Complement to figure 06: same data sliced differently. Use it when you suspect the high- and low-objective basins have systematically different α values — vertical separation between the two colored clouds is the signal.

## 09

No graph produced — analysis 09 (starting-value sensitivity) is text-only.

## 10 — `10_own_elas_boxplot.png`
...
## 11 — `11_multistart_elas_dotplot.png` (one per multi-seed spec)
...
## 14 — `14_spearman_corr_heatmap.png` (Nevo only)
...
## 15 — `15_firm_substitution_heatmap.png` (Nevo only)
...
## 16 — `16_own_stability_heatmap.png`
...
## 17 — `17_cross_stability_heatmap.png`
...
The cross-elasticity analogue of figure 16, restricted to the pairs that matter most in the preferred model.

## 18 — `18_pairwise_mad_heatmap.png`
...
Dark cells = specs that disagree on absolute elasticities (not just rankings). Complement to figure 14 (Spearman), which only sees rank agreement.

## 19 — `19_objective_spec_comparison.png`
...
## 20 — `20_elasticity_pair_across_sims.png`
...
## 21 — `21_elasticity_pair_best_sim_across_specs.png`

`plot_elasticity_pair_best_sim_across_specs` (plot_results.py:1068). 2 × 2 panel like figure 20, but the cross-section is *specifications* — one point per spec, each from that spec's best seed. X-axis = spec (sorted by objective rank); same four panels (`e_jj`, `e_kk`, `e_jk`, `e_kj`). Dashed horizontal line = mean across specs. Pair selected for maximum spec coverage, ties broken by mean cross-elasticity.

Companion to figure 20. Together they answer "do we get the same elasticities when we change (a) the seed within one spec, vs (b) the spec at its best seed?"

## 22 — `22_price_coef_across_sims.png`
...
Use it to read off the distribution of α within the preferred spec — bimodal scatter is the classic two-basin signature, which is the visual counterpart of `06_convergence_audit.txt`.
...
- Heatmaps stop annotating cell values past hardcoded thresholds (e.g., 30 products in figure 12, 25 products in figure 16, 15 specs in figure 18) to keep labels legible.
- Plots that need ≥ 2 specs (figures 16, 17, 18) skip themselves with a `[label] <2 specs` notice when only one spec has data.
